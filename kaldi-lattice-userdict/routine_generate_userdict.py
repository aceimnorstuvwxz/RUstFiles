...
def tpm(bestlatf, kws):
    ...
    for kw in kws:
        spykey = text2simplePinyin(kw) #TODO keyword cache
...
def generate_userdict(fn):
    with open(fn, 'r') as f:
        ...
        ww = filter(lambda x: len(x) > 0, ww)
        print ww

        userdict = []
        
        for w in ww:
            spy = text2simplePinyin(w) 
            userdict.append({"kw":w, "spy":spy})
        
        with open('userdict.json', 'w') as fo:
...
if __name__ == "__main__":
    ...
    USAGE = 'CMD userdict.dat.file'

    generate_userdict(sys.argv[1])
